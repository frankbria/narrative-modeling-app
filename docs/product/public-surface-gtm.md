...
| **Convert** | The 402 body carries `metric`, `limit`, `used`, `resets_at`, `upgrade_available` (`enforcement.py:99-116`); the billing page has an 80 % amber bar and the Upgrade button. **Shipped in #767:** every service throws a shared `ApiError`/`QuotaExceededError` (`lib/services/apiError.ts`), a `PlanLimitDialog` mounted once in the root layout opens on any `quota_exceeded` 402, and a `UsageWarningBanner` surfaces the 80 % signal on every authenticated page. The Upgrade button states its price (#475). | No 80 %-quota email (#494). |
| **Pay** | Checkout, portal and webhook work end to end (#457/#597, test-mode rehearsal in #598). **Shipped in #767:** `/settings/billing` reads `?checkout=success\|cancelled` and shows a confirmation notice (polling for the tier flip on success). | Checkout sends six params: no `customer_email`, `allow_promotion_codes`, `automatic_tax`. No trial/annual/promo/tax decision recorded. |
...
2. Now, in parallel, no decision needed: ~~#767~~ (shipped), #769, #768 AC1–AC3, #476.
