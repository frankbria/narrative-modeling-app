...
## #570 — replacing KEYS with SCAN is a behaviour-preservation exercise, not just an API swap
- **SCAN can return the same key on more than one cursor page; dedup or you double-delete.** A `seen` set makes the deletion idempotent and keeps the return count equal to KEYS's (unique keys removed). This is AC3's real content — a mid-scan add/remove doesn't error because DELETE reports only keys it actually removed.
- **When you move a call from `keys` to `scan`, the test double must gain `scan` too — and carry any guard that lived on `keys`.** The #452 anchored-glob assertion was on `FakeRedis.keys`; delete_pattern no longer calls keys, so the guard had to move onto `scan` (shared `_match` helper) or it would stop protecting the real path. A fake that implements only the old method silently returns 0 and breaks unrelated caller tests (`invalidate_user_cache` fell from 2 to 1).
- **Verify the cursor type against the installed client, not the docs.** The reviewer confirmed redis-py 6.2.0's `parse_scan` returns `int(cursor)`, so `while True: … if cursor == 0: break` terminates correctly — a bytes cursor would never equal the int 0 and loop forever.
- **Check every caller's use of the return value before claiming "count unchanged."** Two of the four callers (transformation, bulk) discard delete_pattern's return entirely, and `_evict_redis`'s `evicted or 0` was already a no-op — so the only count that matters is `invalidate_user_cache`'s sum, which is preserved.
- **A COUNT hint is a per-step work bound, not a result limit** — it caps how many keys Redis examines per cursor step (keeping each step cheap), while the loop drains all pages; 500 keeps the scoped key families to one or two pages.

## #578 — a per-run flake is diagnosed at the fixture, and "identify the cause" means verifying it against the code
- **Per-run vs per-test is the whole diagnosis.** A failure that reproduces across Playwright's in-run retries then passes next run is a run-level state (session warm-up, shared-user status), so raising `retries` does nothing — the fix is at the fixture/session level. Read whether a run's own retries all failed before calling a red E2E Smoke "flaky."
- **Verify the root-cause story against the control flow before shipping it as the diagnosis.** I confidently wrote that an "anonymous-keyed localStorage" timing race caused the /onboarding bounce; the internal reviewer (conf 80) showed the dashboard effect checks `?skipOnboarding` FIRST and `return`s, and grep confirmed nothing auto-navigates off /dashboard to strip the param — so that mechanism can't fire while the param is present. When you can't reproduce a flake, don't assert a mechanism the code contradicts; make the diagnostic REPORT the actual landing URL so the next trip captures the real cause.
- **A fail-fast diagnostic that names the mode satisfies "make it deterministic OR fail with a message naming what was missing."** The bounded retry (≤2 navigations) is the determinism attempt; the mode-naming throw (signin=auth, onboarding=#550, else=unexpected) is the honest fallback. Simulating the 4 landing outcomes in a tiny harness demos the decision logic without the full stack; the PR's own E2E Smoke job is the green-path proof.
- **A dead duplicate fixture is a latent copy of the bug — deprecate it.** `auth.ts`'s `authenticatedPage` (only its type is imported) had the weak skip-less navigation; mark it `@deprecated` so nobody revives the flaky path.

## #580 — a check-then-append across an await needs a lock, and the test must genuinely race
- **The membership check must be atomic with the append.** `if chunk in uploaded_chunks: return` then `append(chunk)` with awaits (file write) in between is a classic read-modify-write race: two coroutines both pass the check, both append, and a DUPLICATE makes `len == total` fire while a real element is missing. The lock has to span from the check through the append; putting the await (file write) inside the hold is required, not a mistake — serializing concurrent different-chunk writes of one session is the acceptable cost.
- **A lazily-created lock registry is race-free on one event loop IF the getter has no await.** `get(sid)` then conditional insert, all synchronous, can't be preempted mid-way — two coroutines can't both create a lock for the same session. Don't add an `await` inside `_chunk_lock`.
- **Dropping a held asyncio.Lock from its registry is safe.** The holder captured the object by reference (`async with`), so an orphan sweep that deletes the dict entry doesn't unlock or corrupt it. That lets one sweep (in the expiry reaper) cover complete/abort/expire without touching each pop site.
- **An asyncio race test only tests the race if the first task runs synchronously to a real suspension point before the second starts.** Because `get_session`/`_chunk_lock` are synchronous, task 1 acquires the lock and reaches `await aiofiles.open` before task 2 runs, so task 2 genuinely blocks on the HELD lock — not avoided by scheduling luck. Verified by mutation: a fresh-lock-per-call getter reproduces `[0,1,1]` + false complete.
- **State the multi-worker ceiling explicitly.** A per-process dict + per-process lock only serializes within a worker; two gunicorn workers racing the same chunk are still unguarded. Name it at the lock and point at the Redis-backed store as the real fix (AC3 asked for exactly this).

## #588 — record the request to make a clamp reversible, and persisting raw input needs a bound
- **A lower-only clamp is a ratchet; to make it reversible, store the target, not just the clamped result.** #455 clamped a key's rate_limit down to the tier ceiling and stored only that. A transient non-entitled status (async_payment_failed → INCOMPLETE → effective_tier FREE) clamped a paying tenant to 1k/hr with no way back. Recording `requested_rate_limit` lets a plan change reconcile BOTH ways: `rate_limit = min(requested, ceiling)` — down on downgrade, back up on recovery.
- **The recorded request is the pivot that distinguishes "we clamped it" from "they set it low."** A key we clamped has requested > current; a key the tenant deliberately set low has requested == the low value, so `min` leaves it. That single rule answers the "how do you tell them apart" question the issue posed — no separate flag needed.
- **A per-row target needs an aggregation-pipeline update, and `$ifNull` covers the rows written before the field existed.** `update_many({user}, [{"$set": {"rate_limit": {"$min": [{"$ifNull": ["$requested_rate_limit", "$rate_limit"]}, ceiling]}}}])` — legacy rows fall back to their current value (lowered, never surprise-raised).
- **Persisting a previously-clamped user input re-opens a validation hole (codex).** The old code stored only `min(request, ceiling)` (small); storing the RAW `request.rate_limit` for restore meant an unbounded `10**100` now reaches insert and 500s on BSON's 64-bit limit. Any field that newly persists raw user input needs its own upper bound (`le=`) so garbage is a 422, not a 500 — the clamp that used to hide the value is gone.
- **`$min` with a finite positive ceiling is safe; verify the ceiling can never be the UNLIMITED (-1) sentinel.** `api_key_rate_limit_ceiling` is `max(1, ...)` and reads every tier through `_env_positive_int`, so `$min` can't zero a key — a rate-limit ceiling is the one place -1 means "brick", not "no limit."

## #602 — enforce a published promise from a stored fact, and separate "entitled" from "paid"
- **A commitment in the Terms is a spec; wire the code to the SAME constant the page renders.** `company.ts refundWindowDays` is the published number; the backend's `REFUND_WINDOW_DAYS` must equal it, guarded by a test that parses the TS file. That is the repo's cross-language single-source pattern (like the Stripe-API-version pin and the nginx-config drift tests) — a shared constant across TS and Python that can't drift because a test fails if it does. Change the policy in one place, both follow.
- **Capture the fact from the event, don't infer it at read time.** "First paid charge" is knowable only when the webhook fires; record `first_paid_at` once (first-write-wins `update_one({field: None})`, which in Mongo matches missing+null) so eligibility is a pure function of stored state, never a Stripe read on the request path.
- **`settled`/entitled is NOT the same as paid (codex + internal reviewer both flagged it).** A checkout with `payment_status == "no_payment_required"` (trial / 100%-discount) is entitled — grant ACTIVE — but no money moved, so it must NOT open a refund window. Reusing the entitlement flag for a payment fact stamped `first_paid_at` on a $0 checkout, and first-write-wins then blocked the real charge from correcting it. Derive a narrower `paid` flag (`"paid"` or `async_payment_succeeded`) for the money fact.
- **A build-vs-document fork with four build-ACs and one fallback-AC is a build.** The issue offered "keep it manual + runbook + close" (AC5) as an alternative, but AC1–4 specified the machinery; under a run-to-completion goal that's not a fatal fork — pick the dominant build path and proceed.
- **When a scoped issue's operator surface depends on a not-yet-built subsystem (#479 admin backend), ship the reachable half and file the rest.** billing/status gives the tenant their own window now; the cross-tenant admin view is filed as a dependent follow-up rather than dragging the admin backend into this PR.

## #626 — a CI "failure" reporter must handle the timeout path, which reports cancelled, not failure
- **`if: failure()` misses timeouts.** A GitHub *job*-level `timeout-minutes` kill, and a concurrency-cancel, both report outcome `cancelled` — and `failure()` is FALSE for a cancellation. A step that only reports on `failure()` stays silent on exactly the long-running case a "review didn't finish" guard exists to catch. Use `!cancelled() && (outcome == 'failure' || outcome == 'cancelled')`: fires on a genuine step failure OR a step timeout (whichever way it's reported), quiet when the whole job is cancelled.
- **A job-level timeout skips the cleanup/reporting step entirely; give the risky step its own shorter step-level timeout.** A job timeout tears down the job immediately — later `if: failure()`/`if: !cancelled()` steps never run. A *step*-level `timeout-minutes` (set below the job's) fails/cancels just that step while the job continues, so the reporting step gets to run. Ordering: step-timeout < job-timeout, both > the work budget (here max_turns).
- **The raised ceiling is not the fix; the visible-failure is.** Bumping `--max-turns` (20→40→60) only moves the cliff — a big enough diff still exhausts it. The durable fix is making the exhaustion VISIBLE (a posted comment), so silence never reads as a clean review. Same class as GLM's startup_failure window (#571).
- **A workflow-only change is verified by actionlint + a condition truth table + the PR's own run.** No unit test exists for a YAML `if:`; enumerate every (job_cancelled × step_outcome) scenario and assert fires/quiet, and let the PR's own bot run exercise the happy path (it posted a real review here, confirming the raised budget still completes). The timeout path's runtime needs a deliberate timeout-test — call that out as the residual.
- **Backticks in a `git commit -m "..."` double-quoted string run command substitution and eat the text.** A message with `` `code` `` silently drops it. Use `git commit -F <file>` (Write the message to a file) for any message containing backticks.

## #629 — a fix copy-pasted into N sibling methods needs the assertion in all N tests
- **Patching a helper "so the test doesn't error" is not a test; assert its call args everywhere the helper is reached.** The fix routed undo/redo/jump through `record_new_file`, but only `undo`'s mock test asserted `assert_called_once_with(dataset, version.s3_url)`; the redo/jump/branching tests merely patched it to keep the mock dataset from being saved. A later divergence in `redo` alone (e.g. `version.file_path`, or dropping the shape copy) would have passed every test. Mutation-check each sibling, not just the first.
- **When the same block is duplicated verbatim across methods, parametrize the real-document test over the methods.** One body run as `undo`/`redo`/`jump_to_position` (`pytest.param(..., id=...)` with a `navigate` lambda) costs three lines and catches a per-method regression the mocks can't. Verified by reverting only `redo`: exactly `test_redo_success` and `[redo]` failed.
- **Bot-review "absent" ≠ "clean" happened again on this PR:** the GLM job died at its step timeout and posted the #626 "did not complete" comment — which is the fix working; treat the verdict as absent and get the third-party review elsewhere (opencode/codex).

## #493 — the gate that can't see the bug class is the bug; and don't touch the tree under a background run
- **A single-tenant test harness cannot observe a cross-tenant regression, no matter how many specs it runs.** `SKIP_AUTH=true` on the e2e backend mapped every identity to one user, so nine P0 isolation failures accumulated under a green gate. The fix was not "more tests" but removing the collapse: the frontend already had two real identities and minted a backend-verifiable JWT; only the backend ignored it. Look for the *structural* reason a gate is blind before adding assertions to it.
- **Prove the guard is a guard with the pre-change runner.** Running the new isolation spec against `git show main:test-e2e.sh` (SKIP_AUTH still on) failed with `Expected [403, 404], received 200` — that is the mutation check for a spec whose "mutation" is an environment variable, and it doubles as the demo evidence.
- **An isolation spec must first prove the owner CAN read.** Otherwise a dead token 401s both tenants and the "cannot read" assertions pass vacuously (the exact trap #683 audits for). Same shape as the #629 lesson: assert the positive path in the same test.
- **Never `git stash` (or otherwise rewrite the tree) while a background e2e/test run is booting from it.** The RED run's Playwright load window overlapped a stash/pop used to get a lint baseline; the run happened to be killed by an unrelated failure, but it could have loaded a mix of old and new files and reported nonsense. Get baselines from `git show main:<file>` / a worktree instead.
- **Local-only env drift masquerades as a code failure:** the browser upload "Failed to fetch" was the backend `.env`'s `BACKEND_CORS_ORIGINS` (ports 3000/3001) rejecting the e2e frontend on :3010 (`OPTIONS … 400`); CI has the var unset (wildcard). Export the origin for the local run; don't edit `.env` or the runner for it. And check `pgrep mongod` on the first `Connection refused` — the WSL box culls it mid-session.
- **`pkill -f "<pattern>"` from inside a Bash tool call matches the tool's own `bash -c` wrapper (its command line contains the pattern) and kills the session shell (exit 144).** Kill by pid from an anchored `pgrep -f "^timeout … codex review"` / `pgrep -f "^node .*codex review"` instead. Second time this bit (see the opencode memory note); now it is here where the merge checklist reads it.
- **A bot review round is per push, so batch the review-driven fixes.** Four claude-review rounds on #750 each surfaced one more small item (cleanup, try-scope, timed-callback hoist); each round cost a full CI cycle plus a codex re-run. Read the whole review, verify every item, and land all in-scope fixes in ONE push.

## 2026-09-15 — issue #552 (staging health probe / PR #754)

- **Revert a mutation with the inverse edit, never `git checkout -- <file>`.** The checkout also discards every *uncommitted* change in that file — it silently threw away the fresh-Mongo fix mid-review-round and the file had to be rewritten from memory. Mutate with `sed`/a Python replace and restore with the exact inverse, or commit first and then checkout.
- **Check the previous deploy's conclusion before reading your own gate as the regression.** The first deploy after #754 went red at the new health gate; the deploy *before* it (db7d82e) had already failed the old bare-`/health` check. `gh run list --workflow=deploy.yml --limit 2` is the 5-second check that separates "my gate caught a live outage" from "I broke staging".
- **A `feature-dev:code-reviewer` subagent can sit idle forever with no report.** It went idle at ~14 min, a `SendMessage` nudge got nothing, and the run waited on it for nothing. Treat the internal review as advisory with a budget: if `ListAgents` shows it idle and no report arrived, record "no report", shut it down, and let the cross-family pass carry the gate.
- **Ask the bot reviewer's premise before its fix.** claude-review's "GitHub runs `run:` under `bash -eo pipefail`" is true only when `shell: bash` is set explicitly; with no `shell:` it is `bash -e`. The finding was wrong but the code was still fragile, so the right move was a shell-option-independent capture (`cmd > file || status=$?`), not the suggested `|| true`.
- **An auto-filed alert issue must keep the phrase the workflow searches for.** `gh issue list --search '"<title>" in:title'` still matched after `[P1]` became `[P1.51]` (GitHub tokenises the prefix away), so the priority backfill was safe — verify with the same search before renaming any bot-managed issue.

## #474 — plan limits / pricing (2026-09-16)
...
## #767 (2026-09-18) — plan-limit dialog, usage banner, checkout confirmation
- **A component added to the root layout meets every e2e spec's catch-all mock.** `stage-transitions.spec.ts` answers unknown routes with `{}`, so the new `UsageWarningBanner` got a `/billing/status` body with no `limits`, threw, and blanked every authenticated page. The new spec passed locally; the unrelated spec failed on all three CI retries. Treat any fetch body read by a layout-wide component as untrusted, and run the whole `chromium-smoke` project locally before pushing a layout change, not only the new spec.
- **`useAsyncData`'s `reload()` returns the hook to `loading`.** A page whose top-level gate is `if (loading) return <Loading/>` loses everything it rendered, a just-shown confirmation included, and a failed refetch lands on the error branch. When the caller already holds the fresh value (a poll result), render that instead of reloading.
- **The pre-commit secret scan flags any line where the word for a login credential is followed by a quoted string of 8+ characters**, e.g. an `expect()` whose first argument is a variable with that name and whose message is long. Rename the variable (`secret`); never bypass the hook. (This entry had to avoid spelling the word out for the same reason.)
- **opencode stalled a fourth time** with the known signature (stream frozen at `prompt_submit`, 180 s). The helper's stall detector fired and exited 75; `codex review --base main` ran as the fallback and completed in about 5 minutes.

## #768 (2026-09-18) — abuse and cost backstops
- **An epic number is not an implementable issue.** `/implementing-issue-plans 764` pointed at an epic with 7 open children, one an owner-only decision. Check the `[EPIC]` title prefix / child checklist first and ask which child. The owner's decision document for #765 lived outside the repo (`/mnt/c/Users/frank/Downloads`, not `~/Downloads` — WSL home ≠ Windows home). It also named a not-yet-cleared product name, which must not reach this public repo; the sign-off was recorded on #765 without it.
- **`ask-opencode.sh "<prompt with the diff inlined>"` dies with `Argument list too long`** on a ~160 KB diff (exec arg limit), and the helper exits 0 anyway. Put the diff in a file and pass `-f <file>` with a short prompt.
- **Both cross-family reviewers can be down at once**: opencode stalled even on a one-word probe, and codex answered `You've hit your usage limit` (no review produced, exit 1). Disclose "Claude-internal only" in the PR body and point to `/code-review ultra`. Do not loop on retries.
- **Tests run while the full gate is in flight share its test database.** `setup_database` wipes every collection at start and end, so mutation or ad-hoc DB runs can flake the gate. Give side runs their own DB: `TEST_MONGODB_DB=narrative_modeling_mut`, and a git worktree for mutations that must not touch the gate's files.
- **A new check in front of an existing validation changes which error wins.** The storage check added ahead of chunked `init_upload` turned the documented oversize 413 into a 402; only the full gate caught it. When adding a guard to a route, run that route's whole existing suite, not only the new tests.

## #769 (2026-09-18) — funnel telemetry
- **Beanie `save()` on a brand-new document is an upsert and fires no `Insert` event.** A model hook on `@after_event(Insert)` silently missed `/datasets/upload`, which creates its `UserData` with `save()`. Hooking `Save` as well fixed the gap but made every later save pay a doomed insert; the right shape is a `@before_event(Insert, Save)` that notes `self.id is None` and an after-hook that acts only on creations. Check how every writer persists before trusting a model hook.
- **next-themes has no theme on the server** (`useTheme().theme` is `undefined` during SSR; it reads localStorage). Rendering selection straight from it is a hydration mismatch on every page. Only a real `renderToString` → `hydrateRoot` test sees it; an RTL `render()` is a client render and stayed green while the component's comment asserted the opposite. The demo's dev-overlay "1 Issue" badge is what surfaced it — read that badge during every UI demo.
- **A stale `.git/index.lock` with no git process running** (an editor's leftover) blocks every commit; confirm `pgrep -x git` is empty, then remove it.
- **Both cross-family reviewers were down again** (opencode stall at `prompt_submit`, codex usage limit). Third time in two days; the internal reviewer plus claude-review caught three real items between them, so the disclosed-internal-only path held up.

## #769 AC4 (2026-09-18) — frontend Sentry
- **A fake Sentry ingest must read the request stream, not `Content-Length`.** The browser SDK posts a sized body, but the Node transport streams gzip chunked, so a Python `rfile.read(Content-Length)` stub saw 0 bytes and the demo reported "the server half sends nothing" for a working setup. Debug the SDK with `SENTRY_DEBUG=1` (it reads the env) before believing a silent path; `Captured error event` plus `Done flushing` meant the harness was the problem.
- **Throwaway demo pages leave `.next/types` pointing at them.** After deleting the pages, `tsc --noEmit` fails on `.next/types/validator.ts` until `.next` is removed. Clear `.next` after any demo build that added pages.
- **The internal reviewer's "Critical" was wrong about the code but right about the gap.** It claimed server events leak headers; the scrubber already replaced `request` wholesale. Asked for a real-SDK test of the server path (`captureRequestError`), which then found the real leak it had also named (`contexts.nextjs.request_path` keeps the query). Disprove with a test, and keep the test.
- **Kimi (opencode, `-m kimi-code-plan-global/kimi-for-coding`) is a working third reviewer** when GLM stalls and codex is out of quota. It ran its own verification (tests, tsc, build) and returned two valid Minors.

## #594 (2026-09-20) — deploying the nginx edge config
- **A pre-symlink `nginx -t` on a not-yet-enabled site passes without parsing the file.** nginx only reads what `sites-enabled` links to, so "install → test → then symlink" tests nothing on a *first* enable; the real failure surfaces after the link, and under `set -e` the script exits leaving a broken symlink that fails every later reload on the box — co-tenant apps included. Enable first, then test once, and roll the symlink back too. Codex found this; my own version had it.
- **`[ -e path ]` is FALSE for a dangling symlink**, which is exactly what a botched rollback leaves. A regression check written with `-e` alone passed under mutation. Use `[ -e ] || [ -L ]`.
- **Mutation-test every check in a script's `--self-check`, not just the risky code.** Two of the checks I wrote were vacuous until mutated (the dangling symlink above, and a "no-op guard" case whose evidence was erased by a later `rm` in the same path). Mutating also showed one *fix* was dead code — an explicit `\r` strip that `[[:space:]]` already covered.
- **Changing when a backup is taken changes what the rollback may assume.** Making the backup conditional on a content change left the rollback's `else` branch deleting a perfectly good file over a failure it had no part in. Branch rollback on "did the target exist", never on "is there a backup".
- **A timestamped backup name at second granularity is not unique.** Two applies in the same second reused one name, so the second overwrote the backup it had just taken — invisible in production (deploys are minutes apart) and only found because a pruning test ran eight applies in one second.
