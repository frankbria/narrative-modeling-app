...
class TestModelRoutes:
    ...
    @pytest.mark.asyncio
    async def test_train_model_gone(self, async_authorized_client, setup_database):
        """POST /api/v1/models/train is removed (#274): it reported
        status="training" but never trained. Now 410 → use /api/v1/ml/train."""
        response = await async_authorized_client.post(
            "/api/v1/models/train",
            json={"dataset_id": "anything", "name": "x", "problem_type": "binary_classification",
                  "algorithm": "Random Forest", "target_column": "target",
                  "feature_columns": ["f1"], "train_test_split": 0.8, "cv_folds": 5},
        )
    ...
    @pytest.mark.asyncio
    async def test_get_performance_metrics_gone(self, async_authorized_client, setup_database):
        """Legacy performance metrics were zero-valued placeholders (the legacy
        train never trained). Removed (#274): 410 → use /api/v1/ml/{id}/evaluation."""
        response = await async_authorized_client.get(
            "/api/v1/models/any_model/performance"
        )
        assert response.status_code == 410
        assert "ml/" in response.json()["detail"]
    ...
    @pytest.mark.asyncio
    async def test_deploy_model_gone(self, async_authorized_client, setup_database):
        """Legacy deploy targeted a ModelConfig the real flow never creates, so it
        404'd every real model. Removed (#274): 410 → use /api/v1/ml/{id}/deploy."""
        response = await async_authorized_client.put(
            "/api/v1/models/any_model/deploy",
