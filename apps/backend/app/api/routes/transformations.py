...
async def get_available_transformations():
    """List the transformation types the engine can actually execute (#499).

    Derived from the engine's registry (``available_transformations``), not a
    hand-maintained list — so the menu can never again advertise types that fail or
    silently do nothing (it previously offered ~22 while the engine implemented 4).
    Imputation (mean/median/mode) is exposed via Fill Missing's ``method`` parameter.
    """
    from app.services.transformation_engine.transformation_engine import (
        available_transformations,
    )

    return [TransformationTypeInfo(**meta) for meta in available_transformations()]
