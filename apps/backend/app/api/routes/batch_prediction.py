...
async def create_batch_job(
    ...
    include_probabilities: bool = Form(
        default=True, description="Include probabilities"
    ),
    include_metadata: bool = Form(default=False, description="Include metadata"),
    include_explanations: bool = Form(
        default=False, description="Include per-prediction feature contributions (#83)"
    ),
...
async def list_batch_jobs(
    ...
    jobs = await batch_service.list_user_jobs(
        user_id=current_user_id, job_type=job_type, status=status, limit=limit
    )
...
@router.get("/stats")
async def get_batch_stats(current_user_id: str = Depends(get_current_user_id)):
    """Get batch processing statistics for the user"""

    # Get all user jobs
    all_jobs = await batch_service.list_user_jobs(
        user_id=current_user_id, limit=1000  # Get all jobs for stats
    )
    ...
    total_predictions = sum(
        j.progress.processed_records for j in all_jobs if j.progress.processed_records
    )

    # Calculate average success rate
    success_rates = [
        j.progress.success_rate for j in all_jobs if j.progress.processed_records > 0
    ]
