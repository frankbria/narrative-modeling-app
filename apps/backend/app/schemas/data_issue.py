...
class DetectionOptions(BaseModel):
    ...
    columns: list[str] | None = Field(None, description="Specific columns to analyze (None = all)")
    sample_size: int | None = Field(None, ge=100, le=100000, description="Sample size for large datasets")
...
class FixPreviewRequest(BaseModel):
    ...
    fix_id: str | None = Field(None, description="Specific fix ID (uses first suggested if not provided)")
...
class FixApplicationRequest(BaseModel):
    ...
    dataset_id: str = Field(..., description="Dataset ID")
    issue_id: str = Field(..., description="ID of the issue to fix")
    fix_id: str | None = Field(None, description="Specific fix ID to apply")
...
class DataIssueResponse(BaseModel):
    ...
    affected_column: str | None = Field(None, description="Affected column name")
...
class IssueDetectionResponse(BaseModel):
    ...
    record_id: str | None = Field(None, description="ID of stored detection record")
...
class FixPreviewResponse(BaseModel):
    ...
    preview_data_before: list[dict[str, Any]] | None = Field(None, description="Data before fix")
    preview_data_after: list[dict[str, Any]] | None = Field(None, description="Data after fix")
...
class FixApplicationResponse(BaseModel):
    ...
    transformation_id: str | None = Field(None, description="Transformation record ID")
    ...
    new_version_id: str | None = Field(None, description="New dataset version ID if created")
    error: str | None = Field(None, description="Error message if failed")
...
class BatchFixResponse(BaseModel):
    ...
    new_version_id: str | None = Field(None, description="New dataset version ID")
