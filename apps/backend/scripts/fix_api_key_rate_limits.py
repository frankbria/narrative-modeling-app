...
async def main() -> int:
    ...
    floor = max(1, limits_for(PlanTier.FREE).api_key_rate_limit)
    ceiling = max(floor, limits_for(PlanTier.ENTERPRISE).api_key_rate_limit)

    collection = AsyncIOMotorClient(uri)[db_name]["api_keys"]

    total = await collection.count_documents({})
    non_positive = await collection.count_documents({"rate_limit": {"$lte": 0}})
    missing = await collection.count_documents({"rate_limit": {"$exists": False}})
    ...
    print(f"rate_limit missing:            {missing}")
    ...
    raised = await collection.update_many(
        {"$or": [{"rate_limit": {"$lte": 0}}, {"rate_limit": {"$exists": False}}]},
