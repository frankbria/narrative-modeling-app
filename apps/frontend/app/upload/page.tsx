...
import { Upload, FileText, Database, AlertCircle, CheckCircle, Shield, Eye, EyeOff, HardDrive, ArrowRight } from 'lucide-react';
...
export default function UploadPage() {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (responseData.status === 'pii_detected' && responseData.requires_confirmation) {
        ...
        // Mark stage as complete and store dataset ID
        completeStage(WorkflowStage.DATA_LOADING, {
          datasetId: responseData.file_id,
          filename: responseData.filename || file.name,
          timestamp: new Date().toISOString()
        });
      } else {
  ...
  const handleConfirmPII = async (maskPII: boolean = true) => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/upload/confirm-pii-upload`, {
        method: 'POST',
        body: formData,
        headers: {
          'Authorization': `Bearer ${await getAuthToken()}`
